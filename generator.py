...
def generate_notes(
    ...
    pitch_idx, offset_idx, duration_idx = seed_tuple
    generated = [(int_to_note[pitch_idx], int_to_offset[offset_idx], int_to_duration[duration_idx])]

    # Prepare initial input tensor: shape (1, 1, 3)
    input_seq = torch.tensor([[[pitch_idx, offset_idx, duration_idx]]], dtype=torch.long, device=device)
